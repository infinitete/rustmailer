...
- `cd server && DATABASE_URL=... ADMIN_TOKEN=... cargo run`: start the backend with required runtime configuration.
...
- `cd frontend && npm run test:security`: verify that external links opened in a new tab include `rel="noopener noreferrer"`.
...
- Any external link using `target="_blank"` must also set `rel="noopener noreferrer"`.
...
- Backend configuration tests should cover both success paths and fail-fast behavior for required environment variables.
- At minimum, run `cd server && cargo test`, `cd frontend && npm run lint`, `cd frontend && npm run build`, and `cd frontend && npm run test:security` before opening a PR.
